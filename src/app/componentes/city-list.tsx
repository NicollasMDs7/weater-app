  ...
  useEffect(() => {
    if (cities.length > 0) {
      ...
      setHistory((prev) => {
        ...
        const newHistory = [newCity, ...updatedHistory].slice(0, 3);
      ...
      {history.length === 0 ? (
        ...
        history.map((city, index) => (
          <div
            key={index}
            className={
              "bg-white/10 rounded-lg p-2 mb-2 flex items-center hover:bg-white/20 transition-colors cursor-pointer"
            }
          ...
          >
            ...
            <div className="ml-2">
              ...
              <div className="flex items-center">
                ...
                <p className="text-xs text-white/80 capitalize ml-2 hidden md:block">
                  {city.description}
                </p>
              </div>
              <p className="text-xs text-white/80 capitalize md:hidden">
                {city.description}
              </p>
            </div>
          </div>
